	...
	if ($error_post == 0) {
		...
		if(empty($error)){
			...
			if(is_array($periodo) and !empty($periodo)){ 
				...
				/* captura */
				$usuarioTMunicipio = " 
					AND JSON_UNQUOTE(JSON_EXTRACT(captura_data, '$.actor_municipio')) = \"".$revision_lineas_servicio_data["actor_municipio"]."\" 
					AND (
						(
						...
						OR (
							...
							JSON_UNQUOTE(JSON_EXTRACT(captura_data, '$.actor_catalogo')) = \"".$revision_lineas_servicio_data["actor_catalogo"]."\"
						)
					)
					AND (
						(
						...
						OR (
							...
							JSON_UNQUOTE(JSON_EXTRACT(captura_data, '$.actor_elemento')) = \"".$revision_lineas_servicio_data["actor_elemento"]."\"
						)
					)
				";
				...
				$captura = $capturaClass->getTablaJSONUnique(
					" 
						AND captura = 'linea'	
						AND JSON_UNQUOTE(JSON_EXTRACT(captura_data, '$.actor')) = ".$revision_lineas_servicio_data["actor"]."
						".$usuarioTMunicipio."
						AND JSON_UNQUOTE(JSON_EXTRACT(captura_data, '$.periodo')) = ".$revision_lineas_servicio_data["idperiodo"]."
						AND activo = 1 
					"
				);
				$arreglos["captura"] = 0;
