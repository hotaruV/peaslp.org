...
if(
	...
	$vars = array(
		...
		'iniciado' => array("leyenda" => "El indicador ya se inició", "tipo" => "radio", "nulo" => 0),
		'avance' => array("leyenda" => "Avance", "tipo" => "texto", "nulo" => 1),
				...
				if(is_array($usuarioT) and !empty($usuarioT)){ 
					...
					if(is_array($captura) and !empty($captura)){ 
						...
						$captura["captura_data"]["indicadores"][$capturar_indicador_data["idindicador"]] = array(
							"iniciado" => $capturar_indicador_data["iniciado"],
							"avance" => $capturar_indicador_data["avance"],
						...
						*/
						...
						foreach($periodos as $key => $periodo){
							if($periodo['idperiodo'] == $capturar_indicador_data["idperiodo"]){
								$encontro = true;
								...
								if(is_array($captura) and !empty($captura)){ 
									...
									$captura['captura_data'] = json_decode($captura['captura_data'], true);
									$captura_data["indicadores"] = $captura['captura_data']["indicadores"];
								}
							}
						}
						/*
					...
					if($resultado > 0){		
						...
						$revision = $revisionClass->getTablaJSONUnique(
							" 
								...
								AND JSON_UNQUOTE(JSON_EXTRACT(revision_data, '$.periodo')) = ".$capturar_indicador_data["idperiodo"]."
								AND activo = 1 
							"
						);
						if(is_array($revision) and !empty($revision)){ 
							...
							if(isset($revision["revision_data"]["indicadores"][$capturar_indicador_data["idindicador"]])){
								$revision["revision_data"]["indicadores"][$capturar_indicador_data["idindicador"]]['actual'] = array(
									"aprobo" => array(
										"v" => array("3"),
										"e" => array()
									),
								...
								$revision["revision_data"]["indicadores"][$capturar_indicador_data["idindicador"]]['historico'][] = $revision["revision_data"]["indicadores"][$capturar_indicador_data["idindicador"]]['actual'];
