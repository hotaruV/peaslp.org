...
if(
	...
	$vars = array(
		...
		'base_arr' => array("leyenda" => "Base - Archivos", "tipo" => "texto", "nulo" => 0),
		'idcaptura' => array("leyenda" => "Indicador", "tipo" => "entero", "nulo" => 0),
		'idindicador' => array("leyenda" => "Periodo", "tipo" => "entero", "nulo" => 0),
			...
			if($arreglos['file'] == 1){
				...
				if($idTablaJSON != NULL){
					...
					if(is_array($obj) and !empty($obj)){
						...
						if($temp[$tabla_data['base_arr']][$tabla_data['idindicador']][$tabla_data['key']]["v"][$tabla_data['index']] == $file_name){
							$temp[$tabla_data['base_arr']][$tabla_data['idindicador']][$tabla_data['key']]["l"][$tabla_data['index']] = $file_src;
						}
						
						$resultado = $tablaJSONClass->updateTablaJSON($idTablaJSON, $obj[$tabla_data['base']], json_encode($temp));
						if($resultado > 0){ 
