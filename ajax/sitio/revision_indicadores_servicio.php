...
if(
	...
	$vars = array(
		'idindicador' => array("leyenda" => "Línea de acción", "tipo" => "entero", "nulo" => 0),
		...
		if(empty($error)){
			...
			if(is_array($periodo) and !empty($periodo)){ 
				...
				if(is_array($revision) and !empty($revision)){ 
					...
					$revision["revision_data"]["indicadores"][$revision_indicadores_servicio_data["idindicador"]]['actual'] = array(
						"aprobo" => $revision_indicadores_servicio_data["aprobo"],
						"retroalimentacion" => $revision_indicadores_servicio_data["retroalimentacion"],
						"quien" => array(
					...
					if(!isset($revision["revision_data"]["indicadores"][$revision_indicadores_servicio_data["idindicador"]]['historico'])){
						$revision["revision_data"]["indicadores"][$revision_indicadores_servicio_data["idindicador"]]['historico'] = array();	
					}
					
					$revision["revision_data"]["indicadores"][$revision_indicadores_servicio_data["idindicador"]]['historico'][] = $revision["revision_data"]["indicadores"][$revision_indicadores_servicio_data["idindicador"]]['actual'];
					...
					//insertar
					$revision_data = array(
						"periodo" => $revision_indicadores_servicio_data["idperiodo"],
						"actor" => $revision_indicadores_servicio_data["actor"],
						"actor_municipio" => (($revision_indicadores_servicio_data["actor_municipio"] != "null")?$revision_indicadores_servicio_data["actor_municipio"]:NULL),
						"actor_catalogo" => (($revision_indicadores_servicio_data["actor_catalogo"] != "null")?$revision_indicadores_servicio_data["actor_catalogo"]:NULL),
						"actor_elemento" => (($revision_indicadores_servicio_data["actor_elemento"] != "null")?$revision_indicadores_servicio_data["actor_elemento"]:NULL),
					);
					
					foreach($default as $key => $valor){ $revision_data[$key] = $valor; }
					
					$revision_data["indicadores"][$revision_indicadores_servicio_data["idindicador"]] = array(
						'actual' => array(
							"aprobo" => $revision_indicadores_servicio_data["aprobo"],
							"retroalimentacion" => $revision_indicadores_servicio_data["retroalimentacion"],
							"quien" => array(
					...
					$revision_data["indicadores"][$revision_indicadores_servicio_data["idindicador"]]['historico'][] = $revision_data["indicadores"][$revision_indicadores_servicio_data["idindicador"]]['actual'];
					
					$resultado = $revisionClass->insertTablaJSON("captura", json_encode($revision_data));
					if($resultado > 0){		
				...
				if($resultado > 0){		
					...
					$email_subject = "Retroalimentación de Indicador: ";
					$email_content = "
						<p>Por medio del presente, <strong>le notificamos que hay nueva retroalimentación para el indicador:  en el marco del seguimiento del Programa de Implementación de la Política Estatal Anticorrupción.</p>
