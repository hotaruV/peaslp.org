...
if(
	...
	if(in_array($usuario_perfil, array(1))){
		$periodos = $periodoClass->getTablaJSONs(
			" AND activo = 1 ",
			" CAST(JSON_UNQUOTE(JSON_EXTRACT(periodo_data, '$.inicia')) AS DATE) ASC "
		);
	} else if(in_array($usuario_perfil, array(5))){
		$periodos = $periodoClass->getTablaJSONs(
			" 
			AND ( 
				NOW() BETWEEN 
				CONCAT(JSON_UNQUOTE(JSON_EXTRACT(periodo_data, '$.inicia_revision')), ' 00:00:00') AND 
				CONCAT(JSON_UNQUOTE(JSON_EXTRACT(periodo_data, '$.termina_revision')), ' 23:59:59') 
			) 
			AND activo = 1 
			",
			" CAST(JSON_UNQUOTE(JSON_EXTRACT(periodo_data, '$.inicia_revision')) AS DATE) ASC "
		);
	}
