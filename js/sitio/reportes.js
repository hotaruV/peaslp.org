...
function reportes_datos(respuesta, tipo){
	...
	//var actores = Object.create(respuesta.actores);
	//var actores = respuesta.actores
	var columnas = [];
	...
	for(i in periodos){
		...
		console.log('periodo_' + i);
		columnas.push({ data: 'periodo_' + i, title: periodo.periodo, "orderable": true, className: "dt-head-center dt-body-center no_wrap" });		
	}
	...
	*/
	...
	for(i in actores){
		...
		if(actor.municipios || actor.catalogos){
			...
			if(actor.hasOwnProperty("captura")){
				...
				if(valores["capturados"] > 0){
					//console.log("capturados", actor.captura.captura_data[tipo]);	
				}
			}
		...
		if(tiene != ""){
			if(actor.municipios){
				for(var z in municipios_arr){
					...
					promerdio_municipios[actor.idactor].push(parseFloat(valores["avance"]));
					valores["avance"] = valores["avance"] + "%"
	...
	for(var j in promerdio_municipios){
		...
		$('#table_data tr span.promedio_mun_' + j).html(((suma / avances.length).toFixed(2)) + "%");
	}
	
	//console.log(promerdio_municipios);
